...
def compare_dataframes(df1, df2, label1, label2):
    ...
    # Normalization: Drop "index" column if present
    # This column is often an artifact of resetting index in pandas/fireducks
    # but doesn't exist in Polars (which doesn't track original row index).
    ...
    try:
        pd_testing.assert_frame_equal(
            ...
            check_index_type=False, # Allow different index types
            check_column_type=False, # Allow int vs float column types
            check_frame_type=False,
            check_names=False,
            rtol=1e-3,  # Relative tolerance for numerical comparisons
            atol=1e-4,  # Absolute tolerance for numerical comparisons
            check_exact=False,  # Allow numerical tolerance
            check_like=True, # Allow different column order (if they match by name)
        )
        ...
        try:
            ...
            # Check that length matches
                ...
                if type_mismatches:
                    ...
                    # align_axis=1 puts compared columns side by side
                    try:
